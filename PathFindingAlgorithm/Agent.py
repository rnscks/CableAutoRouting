...
class Agent:
    ...
    def _InitDistance(self):
        ret = 0
        for i in range(len(self.PathNodeList) - 1):
            ret += self.PathNodeList[i].CenterPoint.Distance(
                self.PathNodeList[i + 1].CenterPoint)
        self.Distance = ret

    def _InitDegree(self) -> None:
        result: float = 0.0
        length: int = len(self.PathNodeList)

        for i in range(length - 2):
            v1 = gp_Vec(self.PathNodeList[i].CenterPoint.XYZ(
            ) - self.PathNodeList[i + 1].CenterPoint.XYZ())
            v2 = gp_Vec(self.PathNodeList[i + 1].CenterPoint.XYZ() -
                        self.PathNodeList[i + 2].CenterPoint.XYZ())
            v2.Reverse()
