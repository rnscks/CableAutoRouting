...
class TessellatorShape:
    def __init__(self, shape: TopoDS_Shape) -> None:
        ...
        self.Mesh_.RandomSampling(10000)
        pass

    def __InitedMesh(self, shape: TopoDS_Shape):
    ...
    def __InitMesh(self, face, mesh: Mesh):
        loc = TopLoc_Location()
...
if (__name__ == "__main__"):
    ...
    shphere = BRepPrimAPI_MakeBox(gp_Pnt(0, 0, 0), gp_Pnt(1, 1, 1)).Shape()
    tlt = TessellatorShape(shphere)
    tlt.Mesh_.DiplayEdgeInMesh(a)
    points = tlt.Mesh_.GetMeshPoints()
    for point in points:
        a.DisplayShape(point)
