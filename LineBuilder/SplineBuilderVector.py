...
class InitalVectorList:
    def __init__(self, gpPntList: list[gp_Pnt]) -> None:
    ...
    def Run(self):
        ...
        for i in range(len(self.GpPntList) - 1):
            gpVectorList.append(
                gp_Vec(self.GpPntList[i], self.GpPntList[i + 1]))

        gpVectorList.append(gp_Vec(self.GpPntList[len(
            self.GpPntList) - 1], self.GpPntList[len(self.GpPntList) - 2]))

        return gpVectorList


class SplineBuilderwithVector:
    def __init__(self, gpPntList: list[gp_Pnt], gpVecList: Optional[list[gp_Vec]], diameter: float = 3) -> None:
        self.Diameter = diameter
        self.GpPntList: list[gp_Pnt] = gpPntList
        self.GpVecList: Optional[list[gp_Vec]] = gpVecList
    ...
    def __InitedSplineShapewithoutVector(self, diameter: float) -> Optional[TopoDS_Shape]:
    ...
    def __InitedSplineShape(self, diameter: float) -> Optional[TopoDS_Shape]:
        if (self.GpVecList is None):
            return self.__InitedSplineShapewithoutVector(diameter)
        ...
        for i in range(1, len(self.GpPntList) + 1):
            ...
            tColgpVec.SetValue(i, self.GpVecList[i - 1])
            tColstdBool.SetValue(i, True)

        tColstdBool.SetValue(len(self.GpVecList), False)
        tolenrance = 1e-3
        interpolate = GeomAPI_Interpolate(tColgpPnt, False, tolenrance)
