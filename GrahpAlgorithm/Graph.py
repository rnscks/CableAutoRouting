...
class AntColonySystem:
    ...
    def Run(self, stop):
        ...
        for _ in range(self.n_iteration):
            ...
            for i in global_shortest_path[0]:
                self.pheromone[i] += (0.1 * (global_shortest_path[1] ** -1))
    ...
    def AntMove(self, pheromone, dist, visited):
        ...
        if (q > self.q0):
            ...
            move = np.argmax(row)
        # local updating rule
        # self.pheromone[move] = (1 - self.decay) * \
        #     self.pheromone[move] + self.decay * self.tu[move]
        return move

    def AntWork(self, start):
        path = []
        visited = set()
        visited.add(start)
        prev = start
